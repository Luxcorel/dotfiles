...
is_installed fd && alias ls-nodemodules="fd --no-ignore --type d --prune node_modules . -x du -sh {} | sort -r --human-numeric-sort"
is_installed fzf && is_installed jq && alias jqi="printf '%s\n' *.json | fzf --disabled --print-query --preview 'jq -C {q} {}'"
